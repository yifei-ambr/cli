...
var AppsDeploy = common.Shortcut{
	...
	Flags: []common.Flag{
		...
		{Name: "file-path", Desc: "publish a single HTML file plus the local files it references, transitively (path relative to the current directory); references resolve inside the file's own directory, and one pointing above it stops the publish — use --dir for a site whose files live in a parent directory; mutually exclusive with --dir"},
